...
def render_module(
    ...
    # Extra stdlib imports for helper annotations and shape-import sidecars.
    typing_imports: list[str] = []
    if "_edges_at" in helpers:
        typing_imports.append("Any")
    typing_import_line = (
        f"from typing import {', '.join(sorted(typing_imports))}"
        if typing_imports else ""
    )
    pathlib_import_line = (
        "from pathlib import Path" if "_HERE" in helpers else ""
    )
    ...
    raw = (
        ...
        + (f"{pathlib_import_line}\n" if pathlib_import_line else "")
        + f"\n"
        + helper_block
