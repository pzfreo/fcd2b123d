...
The translator currently walks the FreeCAD feature tree and emits build123d Python. Starting in tier 3, it also writes a parallel **structured output** — a JSON sidecar that records, in deterministic form, what each translator step did.

The `.py` file is what humans and LLMs edit; the JSON is what dataset tooling and machine consumers read.

### 14.1 Why now, not later

The translator already walks the tree. Capturing additional metadata along the way costs almost nothing. Retrofitting later — once the handlers have grown into a thin pipe of `TranslationUnit` lists — is more painful and risks the metadata being incomplete (some handlers know things at translation time that aren't recoverable from the emitted `.py`, e.g. `Pad.Reversed`, the FreeCAD internal name vs the user-given Label, which sketch a Pad's profile points to).

### 14.2 Accumulator pattern

Handlers take a `TranslationContext` (passed by reference) and accumulate into it:

```python
def translate_pad(pad, ctx):
    ctx.add_step(
        feature_type="pad",
        feature_name=pad.Name,
        depends_on=[pad.Profile[0].Name],
        build123d_code=...,
        renamed_from_default=(pad.Label != pad.Name),
    )
```

At the end of `translate()`, the context is serialised. This is structurally cleaner than passing per-handler return values and avoids retrofitting later.

### 14.3 First-cut schema (deliberately minimal)

```json
{
  "schema_version": "1",
  "exporter_version": "fcstd2b123d-0.1.0",
  "source": {
    "path": "...",
    "category_path": ["Mechanical Parts", "Fasteners", "Bolts"],
    "freecad_version": "1.0.0"
  },
  "summary": {
    "feature_count": 5,
    "primitives_used": ["sketch", "pad", "pocket", "fillet"],
    "parametric_vars": []
  },
  "steps": [
    {
      "step_index": 0,
      "feature_type": "sketch",
      "feature_name": "BaseProfile",
      "freecad_internal_name": "Sketch",
      "depends_on": [],
      "renamed_from_default": true,
      "build123d_code": "...",
      "properties": {
        "volume": null, "surface_area": null,
        "center_of_mass": null, "principal_moi": null
      }
    }
  ],
  "final_code": "..."
...
Per-step `properties` are populated only when the step produces a solid (sketches alone don't have a volume). Using our existing property extractor — same one the tests use — keeps the equivalence story consistent.

**Deliberately excluded from v1 to keep the schema honest:**

- `rationale_hints` — bakes interpretation in too early. The proposed "fillets are detail features" hint encodes intent that fillets have for *topological* reasons (you can't fillet an edge until the edge exists), not because designers consider them ornamental.
- `geometry_hash` — BRep topology hashes are fragile (see ADR-0004); the per-step `properties` snapshot is a stronger equivalence signal and one we already trust.
- `code_cumulative` — redundant data; consumers compute it from the `steps` if needed.
- Natural-language descriptions — those belong in a different file generated by a different pipeline (§15).

### 14.4 CLI surface

```
fcstd2b123d input.FCStd -o output.py --json-out output.features.json
```

Two files, parallel paths. The `--json-out` flag is opt-in for v1.

## 15. Deferred dataset vision

There is a possible future where this project's output isn't just a translator but a **publishable, queryable CAD-reasoning dataset** built by running the translator over the Parts Library and layering LLM-generated reasoning on top.

This is *out of scope for v1* but worth recording in the spec because design decisions now affect it.

### 15.1 Possible shape (when there is a consumer)

```
Mechanical Parts/Fasteners/Bolts/M6x20/
├── M6x20.FCStd              ← upstream (vendored, possibly Git LFS for large files)
├── M6x20.py                  ← deterministic build123d
├── M6x20.features.json       ← deterministic feature record (§14)
├── M6x20.cot.json            ← LLM-generated structured reasoning trace
├── M6x20.cot.md              ← Markdown narrative generated FROM cot.json
└── M6x20.meta.json           ← provenance, hashes, exporter version
```

- Mirror the upstream Parts Library directory structure → trivially discoverable.
- Separate files per concern → different update cadences, different reviewers, clean diffs.
- An `index.jsonl` at the root for tooling/MCP consumers.
- SQLite (optionally with `sqlite-vec`) built from the JSONL on demand for filtered/vector queries.

### 15.2 Principles to lock in now

So the deferred vision isn't poisoned later, design today for:

- **One source of truth per artefact.** If a Markdown CoT exists, it is *generated* from the structured JSON CoT, never hand-edited. Otherwise prose and structure drift and the dataset becomes untrustworthy.
- **Different update cadences live in different files.** Deterministic translator output (`.features.json`) regenerates often during translator development. LLM prose (`.cot.json`, `.cot.md`) regenerates rarely, when better models or prompts become available. They share a path stem but never a file.
- **Every output records its exporter and schema versions.** `exporter_version` and `schema_version` make stale records identifiable for regeneration.
- **A publishable dataset must be self-contained to be citable.** Vendor the FCStd files (with Git LFS for the larger ones), don't submodule. A submoduled upstream can rewrite history and break the dataset.

### 15.3 Deliberately not building (until there is a concrete consumer)

- The 6-file-per-model layout (we have 7 fixtures, not 2,000 — flat tier directories are fine)
- `regenerate.py --all / --changed-since / --missing-cot` (premature without a regen workload)
- JSON Schema files validated in CI (premature schema-as-product)
- MCP server tool design (no consumer asking for queries)
- Vector index / LanceDB choice (no query workload to measure against)
- README-as-marketing for a dataset (no dataset yet)

Tier 3 lands the `--json-out` flag and the minimal §14 schema. Anything beyond that lives in this section until a concrete need surfaces.
